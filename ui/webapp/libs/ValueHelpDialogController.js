...
sap.ui.define([
	 ...
	 */
	var ValueHelpDialog = Dialog.extend("com.bosch.FioriTemplate.libs.ValueHelpDialog", /** @lends sap.ui.comp.valuehelpdialog.ValueHelpDialog.prototype */ {
		metadata: {
		...
		if (this._oTable && this._oTable.isA("sap.ui.table.Table")) {
			...
			if (!this._oTable.getParent() || !this._oTable.getParent().isA("sap.ui.comp.smarttable.SmartTable")) {
				this._oTable.bindAggregation("columns", "columns>/cols", function(sId, oContext) {
					...
					if (sTooltip) {
						oColumn.applySettings({ tooltip: sTooltip });
					}
	...
	ValueHelpDialog.prototype._updateDlgTitle = function() {
		...
		if (this._oTitle) {
			...
			if (sMsg) {
				...
				this._oFilterPanel && this._oFilterPanel.setInnerTitle(sMsg);
			}
		}
	};
	...
	ValueHelpDialog.prototype._updateTableTitle = function() {
		...
		if (iLength > 0 && this.oRows.isLengthFinal && this.oRows.isLengthFinal()) {
			var sRowCount = this._oNumberFormatInstance.format(iLength);
			this._setTableTitle(this._sTableTitle1.replace("{0}", sRowCount));
		} else {
	...
	ValueHelpDialog.prototype._getTabSearchAndSelectText = function() {
		...
		if (oTable) {
			...
			if (oTable.getSelectedIndices) {
				nCountSelectedRows = oTable.getSelectedIndices().length;
			} else if (oTable.getSelectedItems) {
				nCountSelectedRows = oTable.getSelectedItems().length;
			}
	...
	ValueHelpDialog.prototype._registerResizeHandler = function() {
	...
	ValueHelpDialog.prototype._onResize = function() {
		...
		if (oToolbarSpacer) {
			iContentWidth = parseInt(this.getDomRef().style.width);
